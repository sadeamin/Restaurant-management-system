...
class Order_management_system:
    def __init__(self):
        ...
        self.heading = ttk.Label(self.top_order, text="Order Management", foreground="red",
        ...
        self.button_delete_order = Button(self.frame_buttons_order, text="Delete Order", bg="blue", fg="white", command=self.delete)
    ...
    def Add_new_order(self):
        ...
        self.label_total = ttk.Label(self.frame_price, width=20, text="Total price : ", font=("arial", 12))
        ...
        self.label_staff = ttk.Label(self.frame_price, text="Staff name :", font=("arial", 12))
        self.label_staff.grid(row=1, column=0, pady=10, sticky=SW)

        self.label_name = ttk.Label(self.frame_price, text="Sad eamin", font=("arial", 12))
    ...
    def edit_order(self):
        ...
        self.heading = ttk.Label(self.top_add_new_order, text="Edit Order", foreground="red",
                                 font=("arial", 30, "bold italic"))
        self.heading.pack(padx=300)
        ...
        self.edit_order_id = self.treeview.focus()

        self.table_value_4 = self.treeview.item(self.edit_order_id)

        with connection_pool.getconn() as connection:
            with connection.cursor() as cursor:
                cursor.execute("select * from order_items Where order_id = %s", (self.table_value_4["text"],))
                for item in list(cursor):
                    self.treeview_order_2.insert('', "end", text=f"{item[0]}",
                                               values=(f'{item[1]}', f'{item[2]}', f'{item[3]}'))
        ...
        self.button_Order = Button(self.frame_button_c, text="Update", bg="blue", fg="white", command=self.Update)
        self.button_Order.grid(row=0, column=3, ipady=20, ipadx=25, padx=10)
    ...
    def Update(self):
        popup = messagebox.askyesno(title="asking", message="Do you went to update this order")
        if popup:
            with connection_pool.getconn() as connection:
                with connection.cursor() as cursor:
                    cursor.execute("Update public.order set total_price=%s Where id=%s",
    ...
    def Delete_order(self):
        ...
        self.total_price_add = ttk.Label(self.frame_price, text=str(self.total_price),
                                         font=("arial", 12))
        self.total_price_add.grid(row=0, column=1)
        ...
        if self.msg:
            ...
            with connection_pool.getconn() as connection:
                with connection.cursor() as cursor1:
                    cursor1.execute("select id from public.order order by id DESC limit 1 ;")
                            ...
                            with connection.cursor() as cursor1:
                                cursor1.execute(
                                    ...
                                    (self.treeview_order_2.item(i)["text"],
                                     self.treeview_order_2.item(i)["values"][0],
                                     self.treeview_order_2.item(i)["values"][1],
                                     self.treeview_order_2.item(i)["values"][2],
                                     str(self.order_id[0][0])))
    ...
    def delete(self):
        ...
        with connection_pool.getconn() as connection:
            with connection.cursor() as cursor1:
                cursor1.execute("delete from public.order Where id = %s;", (self.table_value_2["text"],))
